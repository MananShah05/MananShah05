<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=280&color=0:0F172A,50:4F46E5,100:8B5CF6&text=Manan%20Shah&fontSize=70&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=GenAI%20Engineer%20•%20Full%20Stack%20Developer%20•%20AI/ML%20Researcher&descAlignY=55" />
</p>

<h1 align="center">
  Hi 👋, I'm Manan Shah
</h1>

<h3 align="center">
  Building AI Products, Intelligent Systems & Premium Software Experiences
</h3>

<p align="center">
  <a href="https://github.com/MananShah05">
    <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&pause=1200&color=8B5CF6&center=true&vCenter=true&width=900&lines=Gen+AI+Engineer;Full+Stack+Developer;AI%2FML+%7C+NLP+%7C+Research;Building+Scalable+Software+Products;Always+Learning+Always+Building" />
  </a>
</p>
---
<p align="center">
  <img src="https://img.shields.io/badge/B.Tech-IT'27-6D28D9?style=flat-square" />
  <img src="https://img.shields.io/badge/DJSCE-Mumbai-4F46E5?style=flat-square" />
  <img src="https://img.shields.io/badge/Mumbai-India-8B5CF6?style=flat-square" />
</p>

<p align="center">
  <a href="https://readme-typing-svg.demolab.com/?font=JetBrains+Mono&size=20&pause=1200&color=8B5CF6&center=true&vCenter=true&width=920&lines=Gen+AI+Engineer;Full+Stack+Developer;AI%2FML+%7C+NLP+%7C+Research;Building+enterprise-grade+software+experiences" target="_blank">
    <img src="https://readme-typing-svg.demolab.com/?font=JetBrains+Mono&size=20&pause=1200&color=8B5CF6&center=true&vCenter=true&width=920&lines=Gen+AI+Engineer;Full+Stack+Developer;AI%2FML+%7C+NLP+%7C+Research;Building+enterprise-grade+software+experiences" alt="Typing SVG" />
  </a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/B.Tech-IT%20'27-6D28D9?style=flat-square" alt="B.Tech IT '27" />
  <img src="https://img.shields.io/badge/DJSCE-Mumbai-4F46E5?style=flat-square" alt="DJSCE Mumbai" />
  <img src="https://img.shields.io/badge/Gen_AI-Engineer-7C3AED?style=flat-square" alt="Gen AI Engineer" />
  <img src="https://img.shields.io/badge/Full_Stack-Developer-6366F1?style=flat-square" alt="Full Stack Developer" />
  <img src="https://img.shields.io/badge/Mumbai-India-8B5CF6?style=flat-square&logo=googlemaps&logoColor=white" alt="Mumbai, India" />
</p>

<p align="center">
  <a href="https://github.com/MananShah05/Manan-Shah-Tech-Portfolio" target="_blank"><img src="https://img.shields.io/badge/Portfolio-View%20Work-8B5CF6?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" /></a>
  <a href="https://www.linkedin.com/in/mananshah001/" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-Connect-4F46E5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="mailto:mananshah05@users.noreply.github.com" target="_blank"><img src="https://img.shields.io/badge/Email-Contact-7C3AED?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
  <a href="https://github.com/MananShah05" target="_blank"><img src="https://img.shields.io/badge/GitHub-Profile-0F172A?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" /></a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=MananShah05&style=flat-square&color=8B5CF6" alt="Profile Views" />
  <img src="https://img.shields.io/github/followers/MananShah05?style=flat-square&label=Followers&color=8B5CF6" alt="Followers" />
  <img src="https://img.shields.io/badge/Stars-14-8B5CF6?style=flat-square" alt="Stars" />
</p>

---

## About

I am a software engineer focused on building reliable, premium-feeling products across AI, full stack development, and data-driven systems. My work blends product thinking with engineering discipline, with a strong emphasis on clean architecture, fast iteration, and user-facing quality.

I build in the intersection of GenAI, NLP, backend systems, and modern frontend engineering. From document intelligence and multimodal research to dashboards, portfolio platforms, and polished interfaces, I enjoy turning complex workflows into software that feels simple, scalable, and production-ready.

**Open To:** software engineering roles, full-stack engineering, AI/ML engineering, applied NLP, product engineering, and research-oriented collaborations.

---

## Tech Stack

### Languages
<p align="center">
  <img src="https://skillicons.dev/icons?i=python,typescript,javascript,java,cpp,c,html,css,sql" alt="Languages" />
</p>

### Frontend
<p align="center">
  <img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,vite,bootstrap,flutter,reactnative" alt="Frontend" />
</p>

### Backend & Databases
<p align="center">
  <img src="https://skillicons.dev/icons?i=fastapi,nodejs,flask,postgres,mongodb,mysql,firebase,supabase" alt="Backend and Databases" />
</p>

### Cloud, DevOps & Tooling
<p align="center">
  <img src="https://skillicons.dev/icons?i=docker,vercel,aws,git,linux,figma,githubactions" alt="Cloud DevOps and Tooling" />
</p>

---

## AI / ML Expertise

| Domain | Proficiency | Details |
|---|---:|---|
| NLP & Document Intelligence | Advanced | Works with extraction pipelines, structured information recovery, embeddings, and language-aware backend workflows. |
| Multimodal Deepfake Detection | Advanced | Research-oriented work combining audio, video, and model-driven analysis for manipulated media detection. |
| Applied Generative AI | Advanced | Builds GenAI features with prompt-driven workflows, model integration, and product-first implementation. |
| ML Modeling & Evaluation | Strong | Comfortable with model selection, feature pipelines, metrics, experimentation, and practical trade-offs. |
| MLOps & Deployment | Strong | Uses FastAPI, Docker, and modern deployment workflows to turn models into usable software services. |
| Data Science & Analytics | Strong | Designs analytics systems, dashboards, and insight layers that support better product decisions. |

---

## Featured Projects

<details>
<summary><b>InsureDoc NLP Extractor</b> — AI-powered document intelligence for structured extraction workflows.</summary>

<br />

| Stack | Scale | Performance | Security | Impact | Repository |
|---|---|---|---|---|---|
| FastAPI, Next.js, PostgreSQL, spaCy, BART, MiniLM | Multi-document extraction pipeline | Async backend flow with reusable NLP stages | Validation-first API design and safe input handling | Reduces manual document review effort | [Search repository](https://github.com/search?q=InsureDoc+NLP+Extractor&type=repositories) |

**Professional explanation:**  
A product-focused extraction system built to transform unstructured documents into reliable structured data. The emphasis is on speed, accuracy, and a clean UX for real-world operational workflows.

</details>

<details>
<summary><b>IndianDeepGuard</b> — multimodal research project for deepfake detection in an Indian context.</summary>

<br />

| Stack | Scale | Performance | Security | Impact | Repository |
|---|---|---|---|---|---|
| Python, multimodal ML, CV, audio analysis, research tooling | Research-grade prototype | Focus on detection robustness and signal fusion | Designed for adversarial and manipulated-media scenarios | Supports misinformation detection research | [Search repository](https://github.com/search?q=IndianDeepGuard&type=repositories) |

**Professional explanation:**  
This project reflects a research mindset and a strong interest in media integrity, model fusion, and applied AI for high-trust environments.

</details>

<details>
<summary><b>Risk-Matrix</b> — enterprise-grade portfolio analytics platform.</summary>

<br />

| Stack | Scale | Performance | Security | Impact | Repository |
|---|---|---|---|---|---|
| TypeScript, analytics pipelines, real-time market data, dashboard UI | Multi-asset portfolio analysis | Computes risk metrics efficiently with clean visualization layers | Focus on data integrity and controlled metric calculation | Helps evaluate drawdowns, volatility, and correlations | [Open repository](https://github.com/MananShah05/Risk-Matrix) |

**Professional explanation:**  
A polished financial analytics product that turns raw market data into practical portfolio insight. The project demonstrates strong product thinking, data handling, and a premium dashboard-first UX.

</details>

<details>
<summary><b>Manan-Shah-Tech-Portfolio</b> — technical portfolio and resume platform.</summary>

<br />

| Stack | Scale | Performance | Security | Impact | Repository |
|---|---|---|---|---|---|
| TypeScript, modern frontend stack, responsive design | Personal brand platform | Lightweight, fast-loading presentation layer | Minimal surface area, clean structure | Serves as the main recruiter-facing portfolio | [Open repository](https://github.com/MananShah05/Manan-Shah-Tech-Portfolio) |

**Professional explanation:**  
A carefully structured portfolio platform that presents skills, projects, and experience with a premium engineering aesthetic. Built to feel clean, credible, and recruiter-friendly.

</details>

<details>
<summary><b>transcend-frames</b> — creative portfolio website combining motion and web engineering.</summary>

<br />

| Stack | Scale | Performance | Security | Impact | Repository |
|---|---|---|---|---|---|
| JavaScript, Next.js, Three.js, motion-focused UI | Creative web experience | Designed for visual fluidity and responsiveness | Standard frontend safety practices | Demonstrates creative engineering and visual polish | [Open repository](https://github.com/MananShah05/transcend-frames) |

**Professional explanation:**  
A visually ambitious web project that showcases interface craft, motion design, and strong front-end execution. It balances storytelling and technical polish.

</details>

<details>
<summary><b>WallCal</b> — focused frontend project with a clean interface-first workflow.</summary>

<br />

| Stack | Scale | Performance | Security | Impact | Repository |
|---|---|---|---|---|---|
| TypeScript, UI-centric frontend stack | Small to medium feature scope | Fast interaction model and clean component structure | Frontend best practices and predictable state flow | Highlights UI discipline and implementation clarity | [Open repository](https://github.com/MananShah05/WallCal) |

**Professional explanation:**  
A compact project that emphasizes structure, usability, and interface quality. It shows attention to component design and frontend implementation detail.

</details>

---


## Achievements

<div align="center">

| Recognition | Details |
|---|---|
| Multimodal Research Focus | Built and explored deepfake detection concepts with an emphasis on real-world media integrity. |
| AI Product Builder | Shipped NLP, analytics, and portfolio systems with a product-first engineering mindset. |
| Full Stack Breadth | Comfortable across frontend, backend, databases, APIs, and deployment-oriented tooling. |
| Strong Technical Identity | Positions engineering work around clarity, trust, and clean execution. |

</div>

---


---

## GitHub Analytics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=MananShah05&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=8B5CF6&icon_color=8B5CF6&text_color=E5E7EB" alt="GitHub Stats" />
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=MananShah05&theme=tokyonight&hide_border=true&background=0D1117&stroke=8B5CF6&ring=8B5CF6&fire=7C3AED&currStreakLabel=8B5CF6" alt="GitHub Streak" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=MananShah05&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=8B5CF6&text_color=E5E7EB" alt="Top Languages" />
</p>
---

## Contribution Activity

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=MananShah05&theme=tokyo-night&hide_border=true&area=true&radius=16&title_color=8B5CF6&color=8B5CF6&line=7C3AED&point=E5E7EB" alt="Contribution Activity Graph" />
</p>

---

## Contribution Snake

<p align="center">
  <img src="https://raw.githubusercontent.com/MananShah05/MananShah05/output/github-contribution-grid-snake.svg" alt="Contribution Snake Animation" />
</p>

---

## Current Focus

```yaml
learning:
  - Advanced system design for scalable products
  - Multimodal AI and applied NLP
  - Cloud-native backend patterns

building:
  - InsureDoc NLP Extractor
  - AI-powered research workflows
  - Premium portfolio-grade web experiences

exploring:
  - Deepfake detection and media forensics
  - Retrieval and document intelligence
  - Product analytics and decision systems

open_to:
  - Software engineering roles
  - Full stack engineering opportunities
  - AI/ML and research collaborations
```

---

## Connect

<p align="center">
  <a href="mailto:mananshah05@users.noreply.github.com" target="_blank"><img src="https://img.shields.io/badge/Gmail-Write%20to%20Me-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail" /></a>
  <a href="https://www.linkedin.com/in/mananshah001/" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="https://github.com/MananShah05" target="_blank"><img src="https://img.shields.io/badge/GitHub-Follow-0F172A?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" /></a>
  <a href="https://github.com/MananShah05/Manan-Shah-Tech-Portfolio" target="_blank"><img src="https://img.shields.io/badge/Portfolio-Explore-8B5CF6?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" /></a>
</p>

---

## Footer

<p align="center"><i>Engineering clarity, trust, and momentum into every product.</i></p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F172A,50:4F46E5,100:8B5CF6&height=140&section=footer" width="100%" alt="Capsule Render Footer" />
</p>
```
